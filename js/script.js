  ...
  $.ajax({
    ...
    success: function (xml) {
      ...
      $(xml).find("item").each(function () {
        ...
        // 取り出したitem1件の中の要素を検索し、最初のテキストを抽出
        // dc\\:title [ \\ ]このバックスラッシュが世界一ややこしい。jQuery(javascript)の解釈だとdc\:titleになって、CSSセレクタの解釈だとdc:titleになる
      ...
      items.slice(0, 20).forEach((x) => {
        const img = x.coverUrl
          ? `<img src="${x.coverUrl}" alt="書影" onerror="this.style.display='none'">`
          : "";
          $("#results").append(`
          <tr>
            <td>${img}</td>
            ...
            <td>${x.price}</td>
            <td>${x.isbnDigits}</td>
            <td><a href="${x.link}" target="_blank" rel="noopener noreferrer">開く</a></td>
          </tr>
        `);
      });
    }
